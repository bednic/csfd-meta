...
pre-defined with an editable default `http://homeserver.local:8080`. Added to
both `strings.po` files. `Settings.relay_url` property returns
...
- `relay/docker-compose.yml`: port map `8080:8080`, `restart: unless-stopped`.
  No volume (stateless).
...
- README: relay build/run on the NAS (docker compose up -d) and setting the
  addon's Relay URL to `http://<nas-host>:8080`.
